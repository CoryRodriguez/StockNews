...
- [x] **EXEC-01**: Bot places market buy orders on Alpaca paper trading API when signal conditions are met
- [x] **EXEC-02**: Bot confirms order fills via Alpaca trading WebSocket stream (real-time, not polling)
- [x] **EXEC-03**: Bot handles partial fills correctly — reconciles position quantity from Alpaca's position endpoint after every fill event
- [x] **EXEC-04**: Bot logs and handles all Alpaca order rejection scenarios without crashing
...
- [x] **EXEC-06**: Bot fires trade execution asynchronously — news handler is never blocked waiting for order placement
...
- [x] **EXIT-06**: Bot monitors open positions via price polling every 5 seconds using existing price tracking infrastructure
