...
**DEFERRAL NOTE — EXIT-02 (Trailing Stop):**
EXIT-02 (trailing stop) is officially deferred to Phase 4 per user decision (CONTEXT.md: "May add trailing stop logic; defer to config too"). Do NOT implement trailing stop exit logic in this file. The `peakPrice` field is tracked in TrackedPosition as groundwork only — it is updated in checkExitConditions but never used to trigger an exit. Phase 4 will wire the actual trailing stop check against peakPrice when the feature is implemented.
...
- Trailing stop NOT implemented (EXIT-02 officially deferred to Phase 4 per user decision) but peakPrice tracked for future use
